...
export default function ServisPage() {
  ...
  async function saveServis() {
    ...
    const { error } = await supabase.from('vehicle_services').insert([{
      vehicle_id: selectedVozilo.id, service_type: form.service_type,
      ...
      status: form.status, notes: notesText, priority: form.priority,
    }])
    ...
    setSaving(false); setForm(emptyForm); setRightTab('istorija'); fetchAll()
  }
  ...
  return (
    <div>
      ...
      {/* ─── VOZILA TAB ─── */}
      {mainTab === 'vozila' && (
        <div style={{ display: 'grid', gridTemplateColumns: selectedVozilo ? '240px 1fr' : '240px 1fr', gap: 16, alignItems: 'start' }}>

          {/* LIJEVO — kompaktna lista vozila */}
              ...
              {loading ? (
                ...
                return (
                  <div key={v.id}
                    ...
                    style={{
                      padding: '7px 10px', cursor: 'pointer', borderBottom: '1px solid #f9fafb',
                      background: isSelected ? '#f0fdf8' : aktivan ? '#fffbeb' : '#fff',
                      borderLeft: `3px solid ${isSelected ? '#1D9E75' : aktivan ? '#f59e0b' : 'transparent'}`,
                      transition: 'all 0.1s',
                    }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexShrink: 0, marginLeft: 4 }}>
                        {aktivan && (
                          <span style={{ fontSize: 8, background: '#FAEEDA', color: '#633806', padding: '1px 5px', borderRadius: 20, fontWeight: 700 }}>U serv.</span>
                        )}
                        {brServisa > 0 && (
                          <span style={{ fontSize: 9, color: '#9ca3af', background: '#f3f4f6', padding: '1px 5px', borderRadius: 10 }}>{brServisa}</span>
                        )}
                      </div>
                    </div>
                    {zadnji && (
              ...
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '12px 16px', marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2, display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    <span style={{ background: selectedVozilo.fleet_status === 'service' ? '#FAEEDA' : '#E1F5EE', color: selectedVozilo.fleet_status === 'service' ? '#633806' : '#085041', padding: '1px 8px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>
                            ...
                            <>
                              {[
                                ...
                                <div key={l as string} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #f3f4f6', fontSize: 12 }}>
                                  ...
                                  <span style={{ color: '#111', textAlign: 'right', maxWidth: 180 }}>{v}</span>
                                </div>
                              ))}
                              {selectedServis.notes && !selectedServis.notes.includes('❌') && (
                                <div style={{ background: '#f9fafb', borderRadius: 6, padding: '6px 8px', fontSize: 11, color: '#374151', marginTop: 8 }}>📝 {selectedServis.notes}</div>
                              )}
                              {selectedServis.notes?.includes('❌') && (
                                <div style={{ marginTop: 8 }}>
                                  <div style={{ fontSize: 11, fontWeight: 600, color: '#374151', marginBottom: 4 }}>Checklist problemi:</div>
                              ...
                              <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
                                <button onClick={() => setEditMode(true)}
                                  style={{ flex: 1, padding: '7px', fontSize: 11, border: '1px solid #d1d5db', borderRadius: 8, background: '#fff', cursor: 'pointer', color: '#374151' }}>✏️ Uredi</button>
                                <button onClick={() => deleteServis(selectedServis.id)}
                                  style={{ padding: '7px 10px', fontSize: 11, border: '1px solid #fecaca', borderRadius: 8, background: '#fff', cursor: 'pointer', color: '#dc2626' }}>🗑️</button>
                              </div>
                            </>
                          ) : (
                            <div>
                              ...
                              <div style={{ marginBottom: 8 }}>
                                ...
                                <textarea value={editForm.description || ''} onChange={e => setEditForm((f: any) => ({ ...f, description: e.target.value }))} style={{ ...inp, minHeight: 50, resize: 'vertical' as const }} />
                              </div>
                              <div style={{ marginBottom: 10 }}>
                                <label style={lbl}>Napomena</label>
                                <textarea value={editForm.notes || ''} onChange={e => setEditForm((f: any) => ({ ...f, notes: e.target.value }))} style={{ ...inp, minHeight: 40, resize: 'vertical' as const }} />
                              </div>
              ...
              {rightTab === 'nova' && (
                <div style={{ maxWidth: 700 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                    <div>
                      <label style={lbl}>Ko je radio</label>
                      <input list="serviseri-nova" value={form.performed_by} onChange={e => setForm((f: any) => ({ ...f, performed_by: e.target.value }))} style={inp} />
                      <datalist id="serviseri-nova">{serviseri.filter(s => s.is_active).map(s => <option key={s.id} value={s.full_name} />)}</datalist>
                    </div>
                    <div>
                      <label style={lbl}>Radionica</label>
                      <input value={form.external_shop} onChange={e => setForm((f: any) => ({ ...f, external_shop: e.target.value }))} style={inp} />
                    </div>
                    <div>
                      <label style={lbl}>Cijena (€)</label>
                    ...
                    <div>
                      <label style={lbl}>Sljedeći servis</label>
                      <input type="date" value={form.next_service_date} onChange={e => setForm((f: any) => ({ ...f, next_service_date: e.target.value }))} style={inp} />
                    </div>
                    <div>
                      <label style={lbl}>Sljedeći (km)</label>
                      <input type="number" value={form.next_service_mileage} onChange={e => setForm((f: any) => ({ ...f, next_service_mileage: e.target.value }))} placeholder="50000" style={inp} />
                    </div>
                  </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: showServiserForm ? '1fr 340px' : '1fr', gap: 20 }}>
          <div>
            {loading ? <div style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>Učitavanje...</div>
              ...
              : serviseri.map(s => (
                <div key={s.id} style={{ background: '#fff', border: `1px solid ${s.is_active ? '#e5e7eb' : '#f3f4f6'}`, borderRadius: 10, padding: 14, marginBottom: 8, opacity: s.is_active ? 1 : 0.6 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
