...
export default function AdminReservationsPage() {
  ...
  const [modal, setModal] = useState<'issue' | 'close' | 'edit' | 'charge' | null>(null)
  ...
  const [washReservation, setWashReservation] = useState<any>(null)
  ...
  async function fetchData() {
    const [{ data: res }, { data: st }, { data: ex }] = await Promise.all([
  ...
  function openModal(type: 'issue' | 'close' | 'edit', r: Reservation) {
    ...
    setModal(type)
    if (type === 'issue') {
      setIssueMode('quick'); setIssuePaymentMode('full')
      setIssueAmount(String(r.total_price))
      setIssuePayment({ method: 'cash', cashAmount: '', cardAmount: '', wireAmount: '' })
    }
    if (type === 'close') {
      setCloseMode('quick'); setCloseStep(hasDebtOrPrepaid(r) ? 'debt_check' : 'surcharges')
  ...
  function openCharge(r: Reservation) {
    setSelected(r)
    supabase.from('reservation_charges').select('*').eq('reservation_id', r.id).order('created_at', { ascending: false }).then(({ data }) => setSelectedCharges(data || []))
    setChargeType('surcharge')
    setChargeItemId(''); setChargeItemName(''); setChargeAmount(''); setChargeComment('')
  ...
  async function handleCharge() {
    ...
    setChargeSaving(false); setModal(null)
    supabase.from('reservation_charges').select('*').eq('reservation_id', selected.id).order('created_at', { ascending: false }).then(({ data }) => setSelectedCharges(data || []))
    fetchData()
  }
  ...
  async function handleWashOrder(skip: boolean) {
    ...
    await supabase.from('wash_orders').insert({
      ...
      wash_type: washType, wash_type_label: wt?.label || washType, price,
      status: 'pending', assigned_to: washAssignedTo,
  ...
  async function handleQuickClose() {
    ...
    if (selected.return_date > today) {
      ...
      updateData.original_return_date = selected.return_date; updateData.return_date = today
    }
    await supabase.from('reservations').update(updateData).eq('id', selected.id)
    setCloseSaving(false); closeModal()
    if (selected) {
      setWashReservation(selected); setWashType(''); setWashAssignedTo('partner')
  ...
  async function handleFullClose() {
    ...
    if (selected.return_date > today) {
      ...
      updateData.return_date = today; updateData.return_time = now.toTimeString().slice(0, 5)
    }
    await supabase.from('reservations').update(updateData).eq('id', selected.id)
  ...
  async function handleCancel() {
    ...
    setCancelSaving(false); setCancelModal(null); setCancelReason('')
    fetchData()
    if (selected?.id === cancelModal) setSelected(null)
  }
    ...
    <div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f9fafb' }}>
                  {['Ref', 'Gost', 'Vozilo', 'Period', 'Iznos', 'Status', 'Plaćanje', 'Kanal', ''].map(h => (
                ...
                {filtered.map(r => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => {
                      ...
                      setSelected(next)
                      if (next) {
                        supabase.from('reservation_charges').select('*').eq('reservation_id', next.id).order('created_at', { ascending: false }).then(({ data }) => setSelectedCharges(data || []))
                      } else { setSelectedCharges([]) }
                    }}
                      ...
                      <td style={{ padding: '10px 12px' }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          ...
                          {(r.status === 'pending' || r.status === 'confirmed') && (
                            <button onClick={() => { setCancelModal(r.id); setCancelReason('') }} style={{ padding: '4px 8px', fontSize: 11, border: '1px solid #fecaca', borderRadius: 6, background: 'transparent', cursor: 'pointer', color: '#dc2626' }}>Otkaži</button>
                          )}
                        </div>
                      </td>
                    </tr>
          ...
          <div style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: 20, fontSize: 13, background: '#fff', alignSelf: 'start' }}>
            ...
            {selectedCharges.length > 0 && (
              <div style={{ marginTop: 12, borderTop: '1px solid #f3f4f6', paddingTop: 12 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 8 }}>Naknadne naplate</div>
                {selectedCharges.map(c => (
                  <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #f9fafb', fontSize: 12 }}>
                    <div>
                      <span style={{ color: '#111', fontWeight: 500 }}>{c.item_name}</span>
                      <span style={{ color: '#9ca3af', marginLeft: 8 }}>{c.agent_name}</span>
                      {c.comment && <span style={{ color: '#9ca3af', marginLeft: 8 }}>· {c.comment}</span>}
                    </div>
                    <span style={{ fontWeight: 600, color: '#BA7517' }}>{c.amount}€</span>
                  </div>
                ))}
              </div>
            )}
            <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {(selected.status === 'confirmed' || selected.status === 'pending') && (
                <>
                  ...
                  <button onClick={() => { setCancelModal(selected.id); setCancelReason('') }} style={{ padding: 9, background: 'transparent', color: '#dc2626', border: '1px solid #fecaca', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>Otkaži</button>
                </>
              )}
              {selected.status === 'issued' && (
                <>
                  <button onClick={() => openCharge(selected)} style={{ padding: 9, background: '#FAEEDA', border: '1px solid #EF9F27', borderRadius: 8, fontSize: 13, cursor: 'pointer', color: '#633806', fontWeight: 600 }}>
                    + Dodaj naplatu
                  </button>
      ...
      {modal && selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 16 }}>
              ...
              {issueMode === 'full' && (
                <div>
                  ...
                  {issuePaymentMode === 'other' && (
                    <input type="number" step="0.01" value={issueAmount} onChange={e => setIssueAmount(e.target.value)} placeholder="Unesite naplaćeni iznos"
          ...
          {/* CHARGE */}
          {modal === 'charge' && (
            <div style={{ background: '#fff', borderRadius: 12, padding: '24px', width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: '28px 24px', maxWidth: 460, width: '100%' }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setWashAssignedTo('partner')} style={{ flex: 1, padding: '9px', border: `1px solid ${washAssignedTo === 'partner' ? '#1D9E75' : '#e5e7eb'}`, borderRadius: 8, background: washAssignedTo === 'partner' ? '#E1F5EE' : '#fff', color: washAssignedTo === 'partner' ? '#085041' : '#374151', cursor: 'pointer', fontSize: 13, fontWeight: washAssignedTo === 'partner' ? 600 : 400 }}>Praonica</button>
                <button onClick={() => setWashAssignedTo('agent')} style={{ flex: 1, padding: '9px', border: `1px solid ${washAssignedTo === 'agent' ? '#1D9E75' : '#e5e7eb'}`, borderRadius: 8, background: washAssignedTo === 'agent' ? '#E1F5EE' : '#fff', color: washAssignedTo === 'agent' ? '#085041' : '#374151', cursor: 'pointer', fontSize: 13, fontWeight: washAssignedTo === 'agent' ? 600 : 400 }}>Agent sam</button>
              </div>
            </div>
            <div style={{ marginBottom: 18 }}>
