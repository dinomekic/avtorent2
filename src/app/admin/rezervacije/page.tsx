...
export default function AdminReservationsPage() {
  ...
  // Otvori modal za prebacivanje u kalendar sa popunjenim podacima
  function openPrebaci(r: Rezervacija) {
    ...
    setRezForm({
      ...
      mjesto_povratka: r.pickup_location || '',
  ...
  async function saveRezervacija() {
    ...
    // Ako je prebacivanje sa sajta — označi originalnu kao issued
    if (prebacujemId) {
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#6b7280', marginTop: 4, margin: '4px 0 0' }}>
            Rezervacije sa sajta · {filtered.length} prikazano
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={inp}>
          ...
          <option value="issued">Prebačeno u kalendar</option>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 340px' : '1fr', gap: 16 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {filtered.map(r => {
                  ...
                  const isPrebacena = r.status === 'issued'
                  return (
                    <tr key={r.id}
                      ...
                      <td style={{ padding: '10px 12px' }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          {!isPrebacena && r.status !== 'cancelled' && r.status !== 'closed' && (
                            <button onClick={() => openPrebaci(r)}
                              style={{ padding: '4px 10px', fontSize: 11, border: '1px solid #1D9E75', borderRadius: 6, background: '#E1F5EE', cursor: 'pointer', color: '#085041', fontWeight: 600, whiteSpace: 'nowrap' as const }}>
                              📅 Kalendar
                            </button>
                          )}
                          {isPrebacena && (
                            <span style={{ fontSize: 11, color: '#1D9E75', fontWeight: 600 }}>✓ Prebačeno</span>
                          )}
        ...
        {selected && (
          <div style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: 20, background: '#fff', alignSelf: 'start' }}>
            ...
            {/* Prebaci dugme u panelu */}
            {selected.status !== 'issued' && selected.status !== 'cancelled' && selected.status !== 'closed' && (
              <button onClick={() => { openPrebaci(selected); setSelected(null) }}
                style={{ width: '100%', padding: '10px', marginBottom: 14, background: '#1D9E75', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                📅 Prebaci u kalendar
              </button>
            )}
            ...
            {([
              ...
              <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #f3f4f6', fontSize: 12 }}>
                ...
                <span style={{ color: l === 'Ukupno' ? '#1D9E75' : '#111', fontWeight: l === 'Ukupno' ? 600 : 400, maxWidth: 180, textAlign: 'right', wordBreak: 'break-word' as const }}>{v || '—'}</span>
              </div>
            ))}
...
export default function AdminReservationsPage() {
  ...
  async function saveRezervacija() {
    ...
    await supabase.from('rezervacije').insert([payload])
    await supabase.from('logovi').insert([{ akcija: `Kreirana rezervacija za ${rezForm.ime_prezime} (${rezForm.br_tablica})` }])
    setSaving(false); setShowRezModal(false); setRezForm(EMPTY_REZ_FORM)
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Pretraži ime, ref, telefon..." style={{ ...inp, width: 240 }} />
                ...
                {filtered.map(r => {
                  ...
                  return (
                    <tr key={r.id} onClick={() => setSelected(selected?.id === r.id ? null : r)}
                      ...
                      <td style={{ padding: '10px 12px', fontSize: 12, color: '#374151' }}>{r.vehicles?.name || '—'}</td>
                      <td style={{ padding: '10px 12px', fontSize: 11, color: '#9ca3af', whiteSpace: 'nowrap' as const }}>{r.pickup_date}<br />{r.return_date}</td>
                      <td style={{ padding: '10px 12px', fontWeight: 600, color: '#1D9E75' }}>{r.total_price}€</td>
                      <td style={{ padding: '10px 12px' }}>
      ...
      {showRezModal && (
        <RezervacijaModal
          form={rezForm} setForm={setRezForm} vozila={vozila}
          onSave={saveRezervacija} onClose={() => { setShowRezModal(false); setRezForm(EMPTY_REZ_FORM) }}
          saving={saving} isNew={isNewRez} title="Nova rezervacija"
        />
      )}
