...
export default function SaradnikLoginPage() {
  ...
  async function handleSessionLogin(session: any) {
    ...
    if (!collab) {
      ...
      setError('Vaš nalog nije registrovan kao saradnik. Kontaktirajte administratora.')
      setLoading(false); setGoogleLoading(false)
      return
    }

    document.cookie = `avtorent-collab-token=${session.access_token}; path=/; max-age=86400`
    document.cookie = `avtorent-collab-id=${collab.id}; path=/; max-age=86400`
    document.cookie = `avtorent-collab-name=${encodeURIComponent(collab.full_name)}; path=/; max-age=86400`
    window.location.href = '/saradnik'
  }

  useEffect(() => {
    supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session) await handleSessionLogin(session)
    })
  }, [])
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    setLoading(true); setError('')
    const { data: collab } = await supabase.from('collaborators').select('id').eq('portal_email', email).eq('is_active', true).single()
    if (!collab) { setError('Nema saradnika sa ovim emailom. Kontaktirajte administratora.'); setLoading(false); return }
    const { error: err } = await supabase.auth.signUp({ email, password })
    if (err) { setError(err.message); setLoading(false); return }
    setSuccess('Nalog kreiran! Provjerite email za potvrdu, pa se prijavite.')
    setLoading(false)
  }
  ...
  async function handleGoogleLogin() {
    ...
    const { error: err } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/saradnik/login`, queryParams: { prompt: 'select_account' } },
    })
    if (err) { setError('Greška.'); setGoogleLoading(false) }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', padding: 36, borderRadius: 12, width: '100%', maxWidth: 400, border: '1px solid #e5e7eb' }}>
        ...
        <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 28 }}>Portal saradnika — {mode === 'login' ? 'prijava' : 'registracija'}</div>
