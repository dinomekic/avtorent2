    ...
    <div style={{ minHeight: '100vh', background: '#f0f6ff' }}>
      <nav style={{ background: '#fff', borderBottom: '1px solid #e5e7eb', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#1a56a0', border: '2px solid #4a90d9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div>
            <div style={{ fontWeight: 800, fontSize: 15, color: '#fff', letterSpacing: -0.5, lineHeight: 1 }}>ADRIA<span style={{ fontWeight: 300, color: '#4a90d9', letterSpacing: 2 }}>DRIVE</span></div>
      ...
      <main style={{ maxWidth: 620, margin: '24px auto', padding: '0 16px 48px' }}>
        <form onSubmit={handleSubmit} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '28px 24px' }}>
          ...
          {partnerName && partnerDiscount > 0 && (
            <div style={{ background: '#E1F5EE', border: '1px solid #4a90d9', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#085041', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              🎁 <span>Kao gost apartmana <strong>{partnerName}</strong> ostvarujete <strong>{partnerDiscount}% popusta</strong> na cijenu najma!</span>
            </div>
          )}

          <div style={{ background: '#f0fdf8', border: '1px solid #4a90d9', borderRadius: 8, padding: '10px 16px', marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                {vehicleImage && (
                  <img src={vehicleImage} alt={vehicleName} style={{ width: 110, height: 72, objectFit: 'cover', borderRadius: 8, flexShrink: 0 }} />
                )}
                {!vehicleImage && (
                  <div style={{ width: 110, height: 72, background: '#f3f4f6', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, flexShrink: 0 }}>🚗</div>
                )}
                <div>
                  <div style={{ fontWeight: 700, fontSize: 15, color: '#111', marginBottom: 4 }}>{vehicleName}</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {vehicleCategory && <span style={{ fontSize: 11, background: '#f3f4f6', borderRadius: 20, padding: '2px 8px', color: '#6b7280', textTransform: 'capitalize' }}>{vehicleCategory}</span>}
                    {vehicleSeats && <span style={{ fontSize: 11, background: '#f3f4f6', borderRadius: 20, padding: '2px 8px', color: '#6b7280' }}>{vehicleSeats} {lang === 'de' ? 'Sitze' : lang === 'en' ? 'seats' : 'sjedišta'}</span>}
                    {vehicleTransmission && <span style={{ fontSize: 11, background: '#f3f4f6', borderRadius: 20, padding: '2px 8px', color: '#6b7280' }}>{vehicleTransmission === 'automatic' ? (lang === 'de' ? 'Automatik' : lang === 'en' ? 'Automatic' : 'Automatik') : (lang === 'de' ? 'Schaltung' : lang === 'en' ? 'Manual' : 'Manuelni')}</span>}
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <span style={{ color: '#1a56a0', fontWeight: 700, fontSize: 14 }}>{Math.round(pricePerDay * (1 - partnerDiscount / 100) * 100) / 100}€/dan</span>
            </div>
          </div>
          ...
          <div style={{ background: '#f9fafb', borderRadius: 8, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ background: '#E1F5EE', border: '1px solid #4a90d9', borderRadius: 6, padding: '8px 12px', fontSize: 13, color: '#085041', fontWeight: 500 }}>
          ...
          {extras.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {extras.map(extra => (
                  <label key={extra.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', border: `1px solid ${selectedExtras[extra.id] ? '#4a90d9' : '#e5e7eb'}`, borderRadius: 8, cursor: 'pointer', background: selectedExtras[extra.id] ? '#f0fdf8' : '#fff' }}>
          ...
          <div style={{ background: '#f9fafb', borderRadius: 8, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 15, fontWeight: 700, padding: '10px 0 0', marginTop: 8, borderTop: '1px solid #e5e7eb', color: '#111' }}>
              <span>{tr.total}</span><span style={{ color: '#1a56a0' }}>{total.toFixed(2)}€</span>
            </div>
            <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 4 }}>Depozit: 300€ (na licu mjesta)</div>
          </div>

          <button type="submit" disabled={submitting} style={{ width: '100%', padding: 13, background: submitting ? '#4a90d9' : '#1a56a0', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer' }}>
