...
type Vozilo = { id: number; license_plate: string; agregirani_2: string; marka: string; model: string }
...
export default function AgentFinansijePage() {
  ...
  const loadAll = useCallback(async (email: string, ime: string) => {
    ...
    // Vozila iz vozila_fleet
    const { data: v } = await supabase.from('vozila_fleet')
      .select('id, license_plate, agregirani_2, marka, model')
      .order('agregirani_2')
    setVozila(v || [])
  ...
  function getUgovorStatus(u: Rezervacija) {
    ...
    return {
      isIzdao, isPreuzeo, fazaReturn, depozit, vracenDep, fali,
      preostaliDug: Math.max(0, ukupno - naplacenoNajam),
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '420px 1fr', gap: 20 }}>
          <div style={card}>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <datalist id="voz-unos">
                {vozila.map(v => <option key={v.id} value={v.license_plate || ''}>{v.agregirani_2} ({v.license_plate})</option>)}
              </datalist>
            </div>
            {needsFoto && (
        ...
        <div>
          {ugovoriLoading ? (
            ...
            <>
                    ...
                    return (
                      <div key={u.id} style={{ ...card, borderLeft: '4px solid #f59e0b' }}>
                        ...
                        <div style={{ fontSize: 12, color: '#1D9E75', marginBottom: 10 }}>
                          Naplaćeno: {u.naplaceno}€ | Duguje: {Math.max(0, (u.ukupno_naplata || 0) - (u.naplaceno || 0)).toFixed(2)}€
                        </div>
              ...
              {ugovori.length === 0 && !ugovoriLoading && (
          ...
          <div style={card}>
            {aktivnoKor ? (
              ...
              <>
                ...
                <div style={{ marginBottom: 10 }}>
                  <label style={lbl}>Vozilo (tablice)</label>
                  <input list="voz-priv" value={privVozilo} onChange={e => setPrivVozilo(e.target.value)} placeholder="Pretraži po tablicama..." style={inp} />
                  <datalist id="voz-priv">
                    {vozila.map(v => <option key={v.id} value={v.license_plate || ''}>{v.agregirani_2} ({v.license_plate})</option>)}
                  </datalist>
                </div>
                <div style={{ marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {(['sve', 'priliv', 'odliv'] as const).map(f => (
                <button key={f} onClick={() => setFilterTip(f)}
                  style={{ padding: '6px 14px', fontSize: 12, border: `1px solid ${filterTip === f ? '#1D9E75' : '#e5e7eb'}`, borderRadius: 20, background: filterTip === f ? '#E1F5EE' : '#fff', color: filterTip === f ? '#085041' : '#6b7280', cursor: 'pointer', fontWeight: filterTip === f ? 600 : 400 }}>
                  {f === 'sve' ? 'Sve' : f === 'priliv' ? '↑ Prihodi' : '↓ Rashodi'}
                </button>
              ))}
            </div>
            <input value={filterSearch} onChange={e => setFilterSearch(e.target.value)} placeholder="Pretraži kategoriju ili vozilo..."
              style={{ ...inp, width: 240 }} />
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f9fafb' }}>
                  {['Kategorija', 'Vozilo', 'Datum', 'Napomena', 'Iznos'].map(h => (
              ...
              <tbody>
                {filteredTrans.slice(0, 50).map(t => {
                  ...
                  return (
                    <tr key={t.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                      ...
                      <td style={{ padding: '10px 14px', color: '#6b7280', fontSize: 11, maxWidth: 200 }}>
                        {t.komentar?.replace(/https?:\/\/\S+/g, '').trim().slice(0, 60)}
                      </td>
                ...
                {filteredTrans.length === 0 && (
                  <tr><td colSpan={5} style={{ padding: 24, textAlign: 'center', color: '#9ca3af' }}>Nema transakcija.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {activeTab === 'potvrde' && (
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#111', marginBottom: 16 }}>
            Potvrde razmjene novca ({pendingTrans.length})
          </div>
              ...
              <div key={t.id} style={{ ...card, borderLeft: '4px solid #f59e0b' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 2 }}>{t.kategorija}</div>
                    {t.komentar && <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>{t.komentar}</div>}
                  </div>
                  <button onClick={() => confirmTrans(t.id)}
                    style={{ padding: '10px 20px', background: '#1D9E75', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                    POTVRDI
                  </button>
                </div>
              </div>
