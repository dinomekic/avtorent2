...
export default function ClientLoginPage() {
  ...
  async function handleGoogleLogin() {
    ...
    const { error: err } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/moje/login`,
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    if (err) { setError('Pogrešan email ili lozinka.'); setLoading(false); return }

    const { data: existing } = await supabase.from('clients').select('id').eq('email', email).single()
    if (!existing) await supabase.from('clients').insert({ email, user_id: data.user.id })
  ...
  return (
    <div style={{ minHeight: '100vh', background: DS.bgPage, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ width: '100%', maxWidth: 400 }}>

        {/* Logo iznad kartice */}
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <a href="/" style={{ fontSize: 22, fontWeight: 700, color: DS.textPrimary, textDecoration: 'none' }}>
            ADRIA<span style={{ color: DS.primaryAccent, fontWeight: 300 }}>DRIVE</span>
          </a>
          <div style={{ fontSize: 11, color: DS.textMuted, letterSpacing: '0.2em', marginTop: 4 }}>KLIJENTSKI PORTAL</div>
        </div>

        <div style={{ background: DS.bgCard, padding: '32px 36px', borderRadius: 16, border: `1px solid ${DS.border}` }}>

          {/* Tab switcher */}
          <div style={{ display: 'flex', background: DS.bgPage, borderRadius: 10, padding: 4, marginBottom: 24 }}>
            {(['login', 'register'] as const).map(m => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  borderRadius: 8,
                  background: mode === m ? DS.primary : 'transparent',
                  color: mode === m ? '#fff' : DS.textSecondary,
                  cursor: 'pointer',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            ...
            <span style={{ fontSize: 12, color: DS.textMuted }}>ili</span>
            <div style={{ flex: 1, height: 1, background: DS.border }} />
          </div>
          ...
          {success ? (
            ...
            <form onSubmit={mode === 'login' ? handleEmailLogin : handleRegister}>
              ...
              <label style={{ fontSize: 12, color: DS.textSecondary, display: 'block', marginBottom: 4 }}>Email adresa</label>
              <input type="email" style={inp} value={email} onChange={e => setEmail(e.target.value)} placeholder="marko@email.com" autoComplete="email" />
              <label style={{ fontSize: 12, color: DS.textSecondary, display: 'block', marginBottom: 4 }}>Lozinka</label>
              ...
              <button
                ...
                style={{
                  ...
                  width: '100%',
                  padding: 11,
                  background: loading ? DS.primaryAccent : DS.primary,
                  color: '#fff',
                  border: 'none',
                  borderRadius: 8,
                  fontSize: 14,
        ...
        {/* Link ispod kartice */}
        <div style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: DS.textSecondary }}>
          <a href="/" style={{ color: DS.textMuted, textDecoration: 'none', fontSize: 12 }}>
            ← Nazad na sajt
          </a>
        </div>
