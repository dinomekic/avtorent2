...
export async function POST(req: NextRequest) {
  try {
    ...
    const { partnerName, partnerEmail, qrCode } = await req.json()

    if (!partnerEmail) {
      return NextResponse.json({ error: 'Email je obavezan' }, { status: 400 })
    }

    // Provjeri da li auth nalog već postoji
    const { data: existingUsers } = await supabase.auth.admin.listUsers()
    const alreadyExists = existingUsers?.users?.find(u => u.email === partnerEmail)

    if (alreadyExists) {
      // Nalog već postoji — samo pošalji welcome email bez lozinke
      const qrLink = `${siteUrl}/?ref=${qrCode}`
      await resend.emails.send({
        from: process.env.FROM_EMAIL!,
        to: partnerEmail,
        subject: `AdriaDrive — Dobrodošli u partnerski program`,
        html: buildWelcomeEmail({ partnerName, partnerEmail, qrCode, qrLink, siteUrl, tempPassword: null }),
      })
      return NextResponse.json({ success: true, existed: true })
    }

    // Kreiraj novi auth nalog sa privremenom lozinkom
    ...
    const { data: authData, error: authErr } = await supabase.auth.admin.createUser({
      ...
      email_confirm: true,
      user_metadata: { full_name: partnerName, role: 'partner' },
    })
    ...
    // Pošalji welcome email sa privremenom lozinkom
    const qrLink = `${siteUrl}/?ref=${qrCode}`
    await resend.emails.send({
      from: process.env.FROM_EMAIL!,
      to: partnerEmail,
      subject: `AdriaDrive — Dobrodošli u partnerski program`,
      html: buildWelcomeEmail({ partnerName, partnerEmail, qrCode, qrLink, siteUrl, tempPassword }),
    })
...
function buildWelcomeEmail(d: {
  partnerName: string; partnerEmail: string; qrCode: string
  qrLink: string; siteUrl: string; tempPassword: string | null
}) {
  return `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
      ...
      <div style="padding:28px 24px">
        <p style="font-size:15px;color:#111">Poštovani/a <strong>${d.partnerName}</strong>,</p>
        ...
        <div style="background:#e8f0fb;border:1px solid #4a90d9;border-radius:10px;padding:20px;margin:20px 0;text-align:center">
          <div style="font-size:14px;font-weight:600;color:#0e2d5e;margin-bottom:6px">Vaši podaci za prijavu</div>
          <div style="font-size:13px;color:#374151;margin-bottom:8px">Email: <strong>${d.partnerEmail}</strong></div>
          <div style="background:#fff;border-radius:8px;padding:12px;margin-bottom:14px;display:inline-block">
            ...
            <div style="font-size:22px;font-weight:bold;font-family:monospace;color:#1a56a0;letter-spacing:2px">${d.tempPassword}</div>
          </div>
          <div style="font-size:11px;color:#9ca3af;margin-bottom:14px">Preporučujemo da promijenite lozinku pri prvoj prijavi.</div>
          <a href="${d.siteUrl}/partner/login" style="display:inline-block;padding:11px 28px;background:#1a56a0;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;font-size:14px">
        ...
        <div style="background:#e8f0fb;border:1px solid #4a90d9;border-radius:10px;padding:20px;margin:20px 0;text-align:center">
          ...
          <div style="font-size:13px;color:#374151;margin-bottom:14px">Prijavite se sa emailom: <strong>${d.partnerEmail}</strong></div>
          <a href="${d.siteUrl}/partner/login" style="display:inline-block;padding:11px 28px;background:#1a56a0;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;font-size:14px">
        ...
        <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:20px;margin:20px 0">
          ...
          <div style="background:#FAEEDA;border-radius:8px;padding:12px;text-align:center;margin-bottom:12px">
            ...
            <a href="${d.qrLink}" style="font-size:13px;font-weight:600;color:#854F0B;word-break:break-all">${d.qrLink}</a>
          </div>
          <div style="text-align:center">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(d.qrLink)}&format=png" width="160" height="160" alt="QR kod" style="display:block;margin:0 auto" />
          </div>
        </div>
...
👉 <a href="${d.qrLink}" style="color:#1a56a0;font-weight:600">${d.qrLink}</a></div>
        </div>
