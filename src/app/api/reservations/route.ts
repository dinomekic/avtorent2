...
export async function POST(req: NextRequest) {
  try {
    ...
    // vehicleId je "marka__model__year" key — traži jedno slobodno vozilo te grupe iz vozila_fleet
    let vehicle: any = null
    let vehicleDbId: any = vehicleId

    const parts = String(vehicleId).split('__')
    if (parts.length === 3) {
      ...
      if (fleetVehicles && fleetVehicles.length > 0) {
        vehicle = fleetVehicles[0]
        vehicleDbId = vehicle.id
      }
    }

    // Ako nije nađeno po key-u, pokušaj direktno po numeričkom id
    if (!vehicle) {
    ...
    // Fallback — nastavi sa mock vozilom da ne blokira rezervaciju
    if (!vehicle) {
      vehicle = {
        id: vehicleId,
        name: body.vehicleName || String(vehicleId).replace(/__/g, ' '),
        price_per_day: 0,
        agregirani_2: body.vehicleName || String(vehicleId).replace(/__/g, ' '),
      }
    }

    const vehicleName = vehicle.agregirani_2 || vehicle.name || `${vehicle.marka} ${vehicle.model}`
    ...
    const finalBasePrice = basePrice ?? days * (vehicle.price_per_day || 0)
    ...
    const { data: existingClient } = await supabase
      .from('clients').select('id, user_id').eq('email', gEmail).maybeSingle()
    ...
    // Kreiraj rezervaciju sa svim poljima
    ...
    if (resErr || !reservation) {
      ...
      return NextResponse.json({ error: 'Greška pri kreiranju rezervacije: ' + resErr?.message }, { status: 500 })
    }
    ...
    try {
      ...
      const ge = guestEmail({
        guestName, vehicleName, pickupDate, returnDate,
      ...
      const ae = adminEmail({
        ...
        vehicleName, pickupDate, returnDate, pickupLocation,
